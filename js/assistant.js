...
function compactCard(p){return `<button class="match-card" data-ai-program="${esc(p.id)}"><span class="match-top"><span class="match-institution ${p.rgsu?'rgsu':''}">${esc(p.shortName)}</span>${icon('arrow')}</span><strong>${esc(programLabel(p))}</strong><span class="match-bottom">${esc(p.level)} · ${esc(p.city)}${p.recordType==='direction'?' · Направление':''}</span></button>`}
